...
use crate::types::{
    ContentAnalysis, Entity, EntityType, MemoryCell, MemoryResult, MemoryType,
    Priority, ProcessedRecall, RecalledMemory, Sentiment,
};
use tracing::debug;
...
impl AIBrain {
    ...
    pub async fn analyze_content(&self, content: &str, context_hint: Option<&str>) -> MemoryResult<ContentAnalysis> {
    ...
    pub async fn process_recall(
        ...
        debug!("Processing recall with {} results", 
        ...
        let semantic = self.filter_and_rank(raw_recall.semantic_layer, 0.7);
        
        // Filter contextual memories
        let contextual = self.filter_and_rank(raw_recall.contextual_layer, 0.5);
        
        // Filter detailed memories
        let detailed = self.filter_and_rank(raw_recall.detailed_layer, 0.3);
    ...
    fn extract_tags(&self, content: &str) -> Vec<String> {
        ...
        let keywords = ["rust", "memory", "ai", "agent", "system", "database", "cache"];
    ...
    fn extract_concepts(&self, content: &str, tags: &[String]) -> Vec<String> {
        ...
        let tech_concepts = ["machine learning", "neural network", "database", "api", "algorithm"];
    ...
    fn analyze_sentiment(&self, content: &str) -> Sentiment {
        ...
        // Positive indicators
        let positive = ["good", "great", "excellent", "happy", "success", "wonderful"];
    ...
    fn generate_suggestions(&self, semantic: &[MemoryCell], contextual: &[MemoryCell]) -> Vec<String> {
        let mut suggestions = Vec::new();
        
        // Suggest exploring related contexts
        let contexts: Vec<String> = contextual
            .iter()
        ...
        if !all_tags.is_empty() {
            suggestions.push(format!("Search for related tags: {}", all_tags[..3.min(all_tags.len())].join(", ")));
        }
...
mod tests {
    ...
    async fn test_memory_type_detection() {
        ...
        let analysis = brain.analyze_content(content, None).await.unwrap();
        assert!(matches!(analysis.memory_type, MemoryType::Procedural { .. }));
